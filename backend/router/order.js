...
import { genarateOrderID, handleOrderInfo,handleOrderInfoWithCost } from "../service/order.js"
...
orderRoute.post("/", verifyToken, verifyCustomer, async (req, res) => {
  try {
    ...
    const { service, sender, receiver, products, origin, destination } =
      req.body;
 ...
 * @access private
 */
orderRoute.get("/", verifyToken, verifyCustomer, async (req, res) => {
  try {
    ...
    const orders = await Promise.all(
      (
        await Order.find({ $and: [{ customer: customerId }, filterCondition] })
          .skip(pageSize * page)
          .limit(pageSize)
          .sort(sortBy)
          .select("-__v")
      ).map(async (order) => await handleOrderInfo(order))
    );
    const length = await Order.find({
      $and: [{ customer: customerId }, filterCondition],
    }).count();
    return sendSuccess(res, "get list of orders successfully.", {
      length,
      orders,
    });
  } catch (error) {
...
orderRoute.get("/:orderId", verifyToken, verifyCustomer, async (req, res) => {
  try {
    ...
    const order = await handleOrderInfoWithCost(
      await Order.findOne({ orderId: orderId }).select("-__v")
    );
...
orderRoute.get("/tracking/:lstOrderId", async (req, res) => {
  try {
    ...
    const orders = await Promise.all(
      (
        await Order.find({ orderId: { $in: lstOrderId } }).select("-__v")
      ).map(async (order) => await handleOrderInfo(order))
    )
 ...
 * @access private
 */
orderRoute.get("/:orderId/feedback",verifyToken,verifyCustomerOrAdmin,async (req, res) => {
    try {
      const orderId = req.params.orderId;
      const order = await Order.findOne({ orderId });
      if (!order)
