...
export const canChangeOrderStatus = async (order, nxtSta) => {
  try {
    ...
    if (curSta === ORDER_STATUS.waiting) {
      ...
      } else if (nxtSta === ORDER_STATUS.accepted && order.route.length >= 0) {
        const numberOfProduct = await Product.find({
          order: order._id,
        }).count();
        const numberOfHandledProduct = await Product.find({
          order: order._id,
          status: PRODUCT_STATUS.already,
        });
        console.log(numberOfHandledProduct);
